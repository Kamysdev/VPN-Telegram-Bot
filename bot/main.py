...
async def start_handler(message: Message):
    ...
    await message.answer_photo(
        photo=photo,
        caption="👋 Добро пожаловать в Интернет-сервис!\n Для оплаты вам необходимо перевести 110р на номер <code>8-983-251-67-44</code> СБП Сбербанк (Константин Андревич С), после чего предоставить скиншот оплаты в этот чат (только изображение). Администратор обработает ваше обращение  предоставит доступ",
    ...
    while True:
        ...
        if not users_to_notify:
            ...
            for user in users_to_notify:
                ...
                await bot.send_message(
                    user['telegram_id'],
                    f"🔔 Напоминаем: срок действия вашей VPN-подписки заканчивается {user['payment_due']}.\nПожалуйста, оплатите, чтобы избежать отключения. Для оплаты переведите 110р на номер <code>8-983-251-67-44</code> СБП Сбербанк (Константин Андреевич С) и отправте скриншот оплаты в чат"
                )
